...
character(len=4) :: diag(0:n_diag) = (/'    ','_d1 ','_d2 ','_d3 ','_d4 ','_d5 ', &
    ...
    integer,parameter :: nx = 3     !! number of points in x
    integer,parameter :: ny = 2     !! number of points in y
   ...
   integer, parameter :: bins=30
    ...
    real(r8) :: asy_table(nx,ny,nz,bins),Rp_BC_table(nz),Rp(bins)
!===============================================================================
CONTAINS
!===============================================================================
...
subroutine modal_aer_opt_init()
   ...
   character(len=128) :: lngname
    ...
    integer :: ii,jj,kk,idi,k1
    real(r8) :: c1,c2,c
   !----------------------------------------------------------------------------
 open (70, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/cabs_table1.txt')
 open (75, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/cext_table1.txt')
 open (80, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/cabs_nobc_table1.txt')
 open (85, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/cext_nobc_table1.txt')
 open (90, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/asy_table1.txt')
  do idi=1,bins ! refr_shell(10)
    ...
    enddo
  enddo
!  read(70,*) ((((cabs_table(ii,jj,kk,idi),ii=1,nx),jj=1,ny),kk=1,nz),idi=1,bins)
!  read(75,*) ((((cext_table(ii,jj,kk,idi),ii=1,nx),jj=1,ny),kk=1,nz),idi=1,bins)
!  read(80,*) ((((cabs_nobc_table(ii,jj,kk,idi),ii=1,nx),jj=1,ny),kk=1,nz),idi=1,bins)
!  read(85,*) ((((cext_nobc_table(ii,jj,kk,idi),ii=1,nx),jj=1,ny),kk=1,nz),idi=1,bins)
!  read(90,*) ((((asy_table(ii,jj,kk,idi),ii=1,nx),jj=1,ny),kk=1,nz),idi=1,bins)
close(70)
close(75)
close(80)
close(85)
...
!           c2=1200.0
!           k1=1
!           do c=c1,c2,(c2-c1)/(nz-1)
!             Rp_bc_table(k1)=c*1e-9
!             k1=k1+1
!           enddo
!           Rp_bc_table(nz)=c2*1e-9

!  open (55, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/r_bc_table1')
!  do jj=1,nz
!   read(55,*)Rp_BC_table(jj)
!  enddo
!  close(55)

!  open (60, file='/nuist/scratch/liuc/ChenGZ/my_cesm_sandbox/cime/scripts/irr/lookuptable/r_coat_table1')
!  do jj=1,bins
!    read(60,*)Rp(jj)
!  enddo
!  close(60)
...
subroutine modal_aero_sw(list_idx, state, pbuf, nnite, idxnite, &
   ...
   real(r8) :: c1, c2, c, Rm(pcols,pver), Rm_bc(pcols,pver)
...
!  call linspace(1.3_r8,1.6_r8,nx,refr_coating)
!  call linspace(0._r8,6.0e-3_r8,ny,refi_coating)
!  Rp_BC_table(1:8)=(/1.0000000e-09,3.9812161e-08,5.2000000e-08,6.3240090e-08,&
!                    7.6866777e-08,8.6000000e-08,1.0300000e-07,1.8418599e-07/)
...
subroutine interp1( x, y, n, t, tmp )
       ...
       if (abs(i-1)<1e-15) then
        tmp = y(1)
       else if (abs( i-(n+1) )<1d-15) then
        tmp = y(n)
       else
...
subroutine opt(refr_shell,refi_shell,dens_wet,Radsurf,vol_fra,mode_num,&
   ...
   real(r8) :: vol_fra,refr_shell,refi_shell,dens_wet,Radsurf,mode_num,pabs,pext,pasm
    real(r8) :: ccabs(bins),ccext(bins),cabs_nobc(bins),cext_nobc(bins),asy(bins)
           ...
           enddo
           Rp(bins)=c2*1e-9
!        Rp(1:10)=(/2.3111111e-08,7.0786262e-08,9.1481322e-08, &
!                 1.1803952e-07,1.3424499e-07,1.5273609e-07, &
!                 1.7783054e-07,2.0439725e-07,2.4737150e-07,4.0661634e-07/)
           sigma_m=1.8
